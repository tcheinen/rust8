...
pub mod cpu {
    ...
    impl CPU for State {
        ...
        fn drw_vx_vy_nibble(&mut self) {
            ...
            let mut loc_x = self.get_vx();
            let mut loc_y = self.get_vy();
            for byte in 0..self.get_n() {
                let y = (self.registers[self.get_vy()] + byte) % DISPLAY_HEIGHT;
                for bit in 0..8u8 {
                    let x = (self.registers[self.get_vx()] + bit) % DISPLAY_WIDTH;
                    let value = (self.memory[self.index as usize + byte as usize] >> (7 - bit)) & 1;
                    self.registers[0xf] |= value & self.memory[y * CHIP8_HEIGHT + x];
                    self.memory[y * CHIP8_HEIGHT + x] ^= value;
                }
            }
        ...
        fn ld_dt_vx(&mut self) {
            self.delay = self.registers[self.get_u8() as usize];
            self.pc += 2;
        }

        /// 0xFx18 - set sound timer to value in Vx
        fn ld_st_vx(&mut self) {
            self.sound = self.registers[self.get_u8() as usize];
            self.pc += 2;
        }
        /// 0xFx1E - add Vx to I
        fn add_i_vx(&mut self) {
            self.index += self.registers[self.get_vx() as usize] as u16;
            self.pc += 2;
        }
        ...
        fn ld_f_vx(&mut self) {
            self.index = FONTSET_START + self.registers[self.get_vx() as usize] as u16 * 5;
            self.pc += 2;
        }
        ...
        fn ld_b_vx(&mut self) {
            let val: u8 = self.registers[self.get_vx() as usize];
        ...
        fn ld_mem_vx(&mut self) {
            for i in 0..self.get_vx() as usize + 1 {
                self.memory[self.index as usize + i] = self.registers[i];
            }
            self.index += self.get_vx() as u16 + 1;
            self.pc += 2;
        }
